...
describe("Claude hook normalization", () => {
  it("maps UserPromptSubmit to the shared start-task protocol", () => {
    ...
    expect(event).toMatchObject({ host: "claude", event: "start_task", externalSessionId: "session-1", cwd: "/repo", payload: { prompt: "Add authentication" } });
  });
});
